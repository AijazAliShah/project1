...
export default class Products extends Component{
  ...
  componentWillMount(){
    const ref = firebase
    .storage()
    .ref("/product_images/"+this.props.match.params.id+".jpg");
    ...
    axios.get(baseUrl+"get/product/"+this.props.match.params.id)
    .then(resp => {
        this.setState({
              ...
              if(this.state.description){
                ...
                if(this.state.image || this.state.oldimage){
                  this.setState({ msg: "", start: true })
                                            ...
                                            axios.put(baseUrl+ 'edit/product/'+this.props.match.params.id,{
                                            ...
                                            .then((resp) => {
                                             ...
                                             if(file){
                                              ...
                                              var storageRef = firebase.storage().ref('product_images/'+ this.props.match.params.id+".jpg");
                                                console.log(file)
                                              //upload file 
                                              ...
                                              task.on('state_changed',
                                                ...
                                                function complete(err){
                                                  that.setState({msg1:"Product edited successfully"})
                                                  setTimeout(() => {
                                                ...
                                                this.setState({msg1:"Product edited successfully"})
                                                  setTimeout(() => {
                                                    window.location = '/admin/Editproduct'
                                                  }, 3000)
                                            }
        ...
        <GridItem xs={12} sm={12} md={12}>
          <Card>
            ...
            <CardBody>
              ...
              <GridContainer md={12}>
                {this.state.showImage ? <img src={this.state.showImage} /> : <img src={this.state.oldimage} />}
              </GridContainer>
            </CardBody>
            <p style={{color: "red", textAlign: "center"}}>{this.state.msg}</p>
                  <p style={{color: "green", textAlign: "center"}}>{this.state.msg1}</p>
            <CardFooter>
              <Button color="primary" onClick={() => this.handleProductAdd()}>Edit Product</Button>
            </CardFooter>
          </Card>
        </GridItem>
      </GridContainer>
