...
export default class StoresRequests extends Component {
    ...
    constructor(props){
      ...
      componentDidMount(){
        axios.get(baseUrl+'get/store/blocked/')
        .then(resp => {
  ...
  render(){
      var data = this.state.allRequests.map((item,index) => (
        [item.storeName, item.ownerName, item.emailAddress, item.storeAddress, <button onClick={() => {
          axios.put(baseUrl+'make/store/active/'+item._id)
      ...
      <GridItem xs={12} sm={12} md={12}>
        <Card>
          <CardHeader color="primary">
            <h4 className={styles.cardTitleWhite}>Blocked Stores</h4>
            <p className={styles.cardCategoryWhite}>
              Here you can see all Blocked Stores
            </p>
          </CardHeader>
          <CardBody>
            <Table
