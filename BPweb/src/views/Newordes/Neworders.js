...
export default class editproducts extends Component {
  ...
  componentDidMount(){
    ...
    axios.get(baseUrl+"get/all/orders/"+localData._id)
        .then(resp => {
            console.log(resp)
    ...
    var data = this.state.orders.map((item,index) => (
      ...
      <div>
       ...
       console.log("Shkjahskldhaklshdk")
           axios.put(baseUrl+"get/orders/isrejected/"+item._id)
           .then(resp => {
                 console.log("upeeee",resp.data)
                 
                 if(resp.data === false){
                   axios.put(baseUrl+"edit/order/accept/"+item._id)
                   .then(resp => {
 ...
 <Button style={{marginLeft: 10}} variant="secondary"onClick={() => 
    ...
    //     var totAmount =0; 
    //      availQuantity = []   
    //      this.setState({todaysOrders: ""})

    //     for(var i=0; i< item.products.length; i++){
    //         console.log("check",totAmount,item.products[i].product,(((item.products[i].product.price - ((item.products[i].product.price * item.products[i].product.discount)/100))) * item.products[i].quantity).toFixed(2) )
    //         totAmount= totAmount + parseFloat((((item.products[i].product.price - ((item.products[i].product.price * item.products[i].product.discount)/100))) * item.products[i].quantity))
            
    //         // await axios.get(baseUrl+"get/product/quantity/"+item.products[i].product.productName)
    //         // .then(async resp => await availQuantity.push({
    //         //   quantity: resp.data,
    //         //   name: item.products[i].product.productName,
    //         //   reqQuantity: item.products[i].quantity
    //         // }))
    //         // .catch(err => console.log(err))
    //       }                      
    //     this.setState({ show: true, rowData: item, selectedProducts: item.products, totAmount: totAmount, availQuantity: availQuantity });

       

    // }}><i className="fa fa-info" style={{fontSize: "20px"}} /></button>
